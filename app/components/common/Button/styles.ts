...
export const Button = styled.button<ButtonProps>`
  ...
  width: ${({ fullwidth, onlyIcon, className }) => {
    if (onlyIcon) return '40px';
    if (className === 'icon') return '40px';
    if (fullwidth || className === 'growingButton') return '100%';
    return 'fit-content';
  }};
  ...
  padding: ${({ size, onlyIcon }) => {
    if (onlyIcon) return '10px 18px';
    if (size === ButtonSizeEnum.M) return '15px 32px 14px';
    return '11px 16px 9px';
  }};
  background-color: ${({ theme, isActive, priority }) =>
    isActive === true && priority === ButtonPriorityEnum.tertiary
      ? theme.buttons.tertiary.backgroundColor
      : null};
  span {
    height: fit-content;
    color: ${({ theme, isActive, priority }) =>
      isActive === true && priority === ButtonPriorityEnum.tertiary
        ? theme.buttons.tertiary.activeSpanColor
        : theme.buttons.tertiary.spanColor};
  }
  svg {
    transition: all 0.2s cubic-bezier(0.2, 0, 0, 1);
  }
